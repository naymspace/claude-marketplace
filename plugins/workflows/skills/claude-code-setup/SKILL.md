...
- **Create `launch.json`** — `.vscode/launch.json` so the app can be started/debugged directly from the IDE (incl. Claude Desktop). No template — it's too project-specific. Inspect the project (runtime, entry point, dev command, env, whether it runs via `docker compose`) and hand-write configurations that fit; for a monorepo add one per service and a compound config. Show the file for review, let the user tweak, then follow-up commit (`chore: add VS Code launch config`), push, amend MR if open. Do NOT add `.vscode/launch.json` to `.gitignore` — it's meant to be shared.
