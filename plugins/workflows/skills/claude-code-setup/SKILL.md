...
- **Create `.claude/launch.json`** — dev-server config for Claude Code Desktop's Preview feature (in-app preview + auto-verify). No template — too project-specific. Inspect the project (runtime, dev command, port, cwd, env, monorepo layout) and hand-write `configurations` entries; for a monorepo add one per service (e.g. frontend + API). Schema: `{ "version": "0.0.1", "configurations": [ { "name", "runtimeExecutable", "runtimeArgs", "port", "cwd", "env" } ] }` — JSONC (comments allowed); use `program`/`args` instead of `runtimeExecutable`/`runtimeArgs` for a node script; `autoPort` for port-conflict handling; top-level `"autoVerify": false` to disable auto-verify. NEVER put secrets in `env` — the file is committed. Show the file for review, let the user tweak, then follow-up commit (`chore: add Claude Desktop launch config`), push, amend MR if open. Do NOT add `.claude/launch.json` to `.gitignore` — it's meant to be shared.
